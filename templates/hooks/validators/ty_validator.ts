/**
 * Ty Type Check Validator - Claude Code PostToolUse Hook
 *
 * Validates Python files after Write/Edit operations using ty.
...
async function main() {
  ...
  const result = await runCommand(["uvx", "ty", "check", filePath]);

  if (result.exitCode === -1) {
    // ty not available, pass through
    console.log(JSON.stringify({}));
    return;
  }
  ...
  if (result.exitCode !== 0) {
    ...
    console.log(
      JSON.stringify({
        decision: "block",
        reason: `Type errors in ${filePath}:\n${output}`,
      })
    );
    return;
  }
